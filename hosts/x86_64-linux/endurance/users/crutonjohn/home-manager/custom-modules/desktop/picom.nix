...
let
  ...
  cfg = config.services.picom-custom;
  opt = options.services.picom-custom;
...
in {
  imports = [
    (mkRemovedOptionModule [ "services" "picom-custom" "refreshRate" ]
      "The option `refresh-rate` has been deprecated by upstream.")
    (mkRemovedOptionModule [ "services" "picom-custom" "experimentalBackends" ]
      "The option `--experimental-backends` has been removed by upstream.")
    (mkRemovedOptionModule [ "services" "picom-custom" "extraOptions" ]
      "This option has been replaced by `services.picom.settings`.")
    (mkRenamedOptionModule [ "services" "picom-custom" "opacityRule" ] [
      "services"
      "picom"
      "opacityRules"
    ])
  ...
  config = mkIf cfg.enable {
    services.picom-custom.settings = mkDefaultAttrs {
      # fading
      fading = cfg.fade;
    ...
    systemd.user.services.picom = {
      ...
      Service = {
        ExecStart = concatStringsSep " " ([
          "${pkgs.nix}/bin/nix run github:guibou/nixGL#nixGLIntel --"
