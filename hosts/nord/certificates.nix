...
let
  ...
  stepCertRenewService = {
    description = "Renew Step CA certificate for nord.heyjohn.family";
...
{
  services.step-cert-renew = stepCertRenewService;
  ...
  systemd.timers.step-cert-renew = {
    ...
    serviceConfig = stepCertRenewService.serviceConfig; # Reuse the service configuration
  };
